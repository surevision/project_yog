...
public class UISetTitle : UISetBase {
    ...
    public override void start() {
        ...
        InputManager.clear();
        // 加载菜单节点
        this.menuNode = GameObject.Instantiate<GameObject>(Resources.Load<GameObject>("prefabs/ui/menus/TitleMenu"));
        this.menuNode.transform.SetParent(this.messenger.uiNode.transform);
        this.menuNode.name = "TitleMenu";
        // 读取节点信息
		...
		while (true) {
			...
			if (InputManager.isTrigger(InputManager.GameKey.C)) {
                if (this.menuIndex == 0) {
                    ...
                    ((SceneTitle)SceneManager.Scene).startNewGame();
